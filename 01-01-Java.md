...
自定义ClassLoader：可实现 `public Class\<?\> loadClass(String name) throws ClassNotFoundException` (打破双亲委派) 或 `protected Class\<?\> findClass(String name) throws ClassNotFoundException`(不打破双亲委派)。
