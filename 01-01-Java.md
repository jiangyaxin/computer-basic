...
* `-XX:G1HeapRegionSize=n`： region 的大小，默认为2M。该值将是 2 的幂，范围可以从 1MB 到 32MB。目标是使最小的 Java 堆拥有大约
  2048 个区域。
...
* `-XX:InitiatingHeapOccupancyPercent=45`：触发标记周期的 Java 堆占用阈值，默认占用率为整个 Java 堆的 45%。
