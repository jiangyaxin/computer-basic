...
Young-only阶段: 从普通的young GC开始,把对象晋升到old区,当old区的占有率达到一个阈值的时候就开始young-only阶段到space-reclamation阶段的转换,就开始
Concurrent Start的young GC。
Concurrent Start 会进行全局并发标记，找出当前old区中所有的存活的对象以备随后的 space-reclamation阶段来使用。在这期间可能发生普通
young GC，所以全局并发标记还会经历两个 STW ： 重新标记、清理。
...
由 Concurrent Start的young GC 触发，如果清理之后，JAVA堆达到阀值（默认自适应，使用 `-XX:-G1UseAdaptiveIHOP`
...
* `-XX:InitiatingHeapOccupancyPercent=45`：触发标记周期的 Java 堆占用阈值，默认占用率为整个 Java 堆的
  45%，与 `G1MixedGCLiveThresholdPercent` 搭配使用。
* `-XX:G1HeapWastePercent=10`：当回收百分比小于堆浪费百分比时，不会启动混合垃圾回收周期，默认值为 10%。
...
* `-XX:G1ReservePercent=10`：保留内存的百分比，默认值为 10%。
...
* `-XX:G1MixedGCLiveThresholdPercent`: 只有存活对象低于该值的Region才可以被MixedGC回收。
