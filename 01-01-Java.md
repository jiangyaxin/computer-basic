...
CompletableFuture 完成有两个方法 `public boolean complete(T value)`、`public boolean completeExceptionally(Throwable ex)` 分别对应正常结束和异常结束。获取结果有三个方法 `get()`（阻塞），`get(long timeout, TimeUnit unit)`（阻塞一段时间）,`getNow(T valueIfAbsent)`（如果没有获取到值 则使用默认值）
