...
2. 使用 `java.lang.reflect` 包的方法对类进行反射调用时，如 `Class.forName("...")`、`newInstance()` 等等。如果类没加载，需要触发其加载。
3. 初始化一个类，如果其父类还未加载，则先触发该父类的加载。
4. 当虚拟机启动时，用户需要定义一个要执行的主类 (包含 main 方法的那个类)，虚拟机会先加载这个类。
