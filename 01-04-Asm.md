...
独立的jar包，运行时使用命令 `java -javaagent:代理程序.jar[=arg1:value1,arg2:value2] 目标程序.jar`,`arg1:value1,arg2:value2`
会作为参数传入到下面的方法参数 agentArgument，也可不传参数。

2. 代理程序需要实现 `public static void premain(String agentArgument,Instrumentation instrumentation) throws Exception`
   ，JVM 在类加载时先执行 premain 方法，再执行main方法，可以通过 Instrumentation 替换字节码修改类。
...
独立的jar包，实现 `public static void agentmain(String agentArgument,Instrumentation instrumentation) throws Exception`
接口，并在接口中调用 `Instrumentation#etransformClasses`。
...
3. 跨进程通信使用 `VirtualMachine#attach` 访问目标JVM。
...
* ClassReader: 负责读取字节码，每当有事件发生时，触发相应的ClassVisitor、MethodVisitor。
* ClassVisitor: `ClassReader#accept` 需要传入 ClassVisitor 对象，ClassReader遇到不同的节点时会调用 ClassVisitor
  不同的visit方法，比如 visitAttribute 、visitField、visitMethod 等。
* ClassWriter：ClassVisitor的子类，ClassReader 触发 `ClassWriter#visitXXX`
  方法时会将字节码记录在内部数据结构，所以调用  `ClassWriter#visitXXX` 等于将字节码保存在
  中ClassWriter，创建一个新类时等于按顺序调用 `ClassWriter#visitXXX`，最终修改的字节码可以通过 `ClassWriter#toByteArray`
  返回。
...
1. 创建
   ClassWriter，`ClassWriter cw = new ClassWriter(ClassWriter.COMPUTE_FRAMES | ClassWriter.COMPUTE_MAXS)`,`ClassWriter.COMPUTE_FRAMES | ClassWriter.COMPUTE_MAXS`
   表示自动计算操作数栈、局部变量表、StackMapFrames，使用 `MethodVisitor#visitMaxs(0,0)` 触发计算。
...
* onMethodExit：正常退出和异常退出时被调用，正常退出指的是遇到 `RETURN`、`ARETURN`、`LRETURN`,异常退出时遇到 `ATHROW`
  指令，有异常抛出方法返回。
...
每个需要编辑的class对应一个 CtClass，储存在 ClassPool 中，字段和方法对应模型为 CtField 、CtMethod。
