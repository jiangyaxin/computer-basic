...
Seata 目前支持 `AT`模式、`XA`模式、`TCC`模式、`SAGA`模式。

`XA`模式: 利用事务资源(数据库、消息服务等)对`XA`协议的支持，以协议的机制来管理分支事务，可以做到全局事务一致性，但事务过程中两阶段都会锁定资源，锁定时间长性能查，当资源失联时协议将阻塞。

`AT`、`TCC`、`SAGA`模式都属于补偿式的事务。

`AT`、`TCC` 在一阶段就完成事务的提交，释放资源，性能好。

`AT` 模式需要依赖本地事务，使用全局锁实现读写隔离，没有代码入侵，框架通过拦截SQL生成`Undolog`表记录,自动完成提交和回滚。

`TCC` 模式不依赖本地事务，可以用于非事务型数据库，代码侵入，没有`Undolog`表记录和全局锁，性能最好，但需要考虑提交和回滚失败的情况，做好幂等处理。

`TCC` 存在问题，都是由于网络分区、故障等分布式问题引起：

* 幂等： 当事务中心没有收到分支事务的响应，需要进行重试，需要分支事务支持幂等，通过开启 `tcc_fence_log`,提交事务时往`tcc_fence_log`插入一条记录，供后面重试判断。
* 空回滚：多个分支事务时中存在有分支事务故障，需要对事务进行回滚，通过在一阶段时往`tcc_fence_log`插入一条记录，回滚时只回滚`tcc_fence_log`存在对应记录的分支。
* 悬挂：由于网络问题，`Rollback`指令先于`Try`指令到达，导致`Try`预留的资源无法回滚，通过在执行`Rollback`方法时先判断`tcc_fence_log`是否存在记录，如果没有不执行回滚，并且插入一条记录，等到`Try`指令到达时准备插入记录时会因为主键冲突而失败。
