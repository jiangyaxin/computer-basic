...
- `if`：`test`使用 OGNL 表达式，`property ！= null and property != ''` 用于字符串判断不为空。
...
`SimpleExecutor`和`ReuseExecutor` 可以正确返回，`BatchExecutor` 返回id为null。
...
8. 在 Mapper 中如何传递多个参数?
   ...
   ```
    ...
    - 第三种，传递多个参数，不使用`@Param` 注解，按照参数在方法方法中的位置，从 1 开始，逐个为`#{param1}`、`#{param2}`、`#{param3}` 不断向下：
...
16. 启动流程：
    * 读取 `mybatis-config.xml` 创建 `SqlSessionFactory`，由`SqlSessionFactoryBuilder`创建,并且负责xml读取。
    * 读取 `mapper-location` 的 xml文件，生成`MapperStatement`，储存在`SqlSession`中，`MapperStatement`包含`MapperProxyFactory`,可根据 `Method` 生成对应的 `MapperProxy` 代理。
    * `MapperScan` 通过 `import + ImportBeanDefinitionRegistrar + BeanDefinitionRegistryPostProcessor`方式启动扫描 `@Mapper` 注解的类，生成 `MapperFactoryBean` ，`MapperFactoryBean` 可通过 `SqlSession` 获取 `MapperProxy`。
...
Stream：使用完成后需要对流进行关闭，可使用`try-with-resources`底层使用滚动结果集获取，可以用于大数据量查询，需要将结果集设置为 `TYPE_FORWARD_ONLY`(JPA已默认设置) 、 `CONCUR_READ_ONLY` 并且设置 `FETCHSIZE`，否则会内存溢出。 
...
2. `@PrePersist`、`@PostPersist`：
    ...
    - `@PrePersist`事件在调用persist()方法后立刻发生，此时的数据还没有真正插入进数据库。
...
4. `@PreRemove`、`@PostRemove`：
    ...
    - `@PreRemove`事件在实体从数据库删除之前触发，即在调用remove()方法删除时发生，此时的数据还没有真正从数据库中删除。
...
默认情况下，在启动期间，Hibernate会在创建持久化单元时为每个持久化类创建SQL，当字段特别多的表，可以使用 `@DynamicInsert` 和`@DynamicUpdate`来禁用启动时创建，而是在需要时生成SQL，UPDATE将仅包含具有更新后的值的列，而INSERT将仅包含非空列。
