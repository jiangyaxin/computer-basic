...
Stream：使用完成后需要对流进行关闭，可使用`try-with-resources`底层使用滚动结果集获取，可以用于大数据量查询，需要将结果集设置为 `TYPE_FORWARD_ONLY`(JPA已默认设置) 、 `CONCUR_READ_ONLY` 并且设置 `FETCHSIZE`，否则会内存溢出： MYSQL：设置连接属性 `useCursorFetch=true`,
