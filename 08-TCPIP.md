...
慢启动就是定义在发送端定义一个拥塞端口，拥塞端口在每次通信后按照一定的算法扩大，在每次发送数据包时，使用拥塞端口和接收端通知窗口做比较，使用较小的窗口发送。
...
* FTP：在两个相连的计算机之间镜像文件传输的协议，使用两天TCP连接，一条用于控制，端口21，例如登录用户名密码、发送文件名称等，一条用于数据传输，端口随机，由于数据传输的TCP连接时按照与控制连接相反方向建立，所以使用NAT连接外部FTP服务器是，无法建立数据传输用的TCP通道，需要更改建立连接的方向才行。
