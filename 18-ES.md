...
* 通用参数
    ...
    2. `+1h` 增加一个小时 `-1d` 减少一个小时 `/d` 上一个小时，所支持的时间单位为：y（年）、M（月）、w（周）、d（日）、h（小时）、m（分钟）、 s（秒），表达式设定的日期为 now 或者日期字符串加||。
...
* `saturation`：默认值，score区间(0,1)，打分公式为 `S / (S + pivot)`，S是字段的值，`pivod`是score分界值，默认使用字段近似平均值，当S值大于pivot时，`score>0.5` ，当S值小于pivot时，score<0.5 。
