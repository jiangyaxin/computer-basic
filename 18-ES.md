...
| store      | 默认为 `false` , 默认存储在`_source`字段，当`true` 时，会独立于`_source`建立额外的存储，如果需要从文档中提取，会减少计算                        |                                |
...
| include_in_all   | 决定字段是否应该被包含在`_all` 字段中。接受 `true` 或 `false` 参数。<br />如果索引被设置为 `no` 或者父对象字段设置 `include_in_all` 为 `false`，<br />参数默认值为 `false`；其他情况下，默认值为 `true`。 |
...
12. `text`：在被搜索之前通过分词器进行分词，然后形成倒排索引，不适合排序、聚合计算。
    ...
    | include_in_all         | 决定字段是否应该被包含在`_all` 字段中。接受 `true` 或 `false` 参数。<br />如果索引被设置为 `no` 或者父对象字段设置 `include_in_all` 为 `false`，<br />参数默认值为 `false`；其它情况下，默认值为 `true`。        |
