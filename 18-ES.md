...
* 分片：单个`Lucene`(全文搜索引擎工具包)实例，文档储存在分片中，分片分配在集群的节点，当集群扩容或缩小时，ElasticSearch 将自动在节点间迁移分片，以使集群保持平衡。
...
* 文档：储存在 `ElasticSearch` 中的一个JSON格式的字符串，代表一行数据，对应关系型数据库的`Row`。每个储存在索引中的文档都有一个类型和一个ID，每个文档都是一个JSON对象，储存了零个或多个字段，原始的JSON文档被储存在 _`source` 的字段中，当搜索文档时默认返回的就是这个字段。
...
* 批量提交
* 合理设置段合并速度
* 减少`refresh`次数(从内存刷新到文件缓存中)
* 加大`flush`设置(文件缓存多大的值之后从文件缓存中`flush`到磁盘上)
* 减少副本数量
* 优化磁盘
...
使用 `Bulk` 来进行批量提交，每次提交的数据量受到文件文件大小、网络情况、数据类型、集群状态等因素影响，可通过测试设置最佳数据量。

Bulk 默认设置批量提交的数据量不能超过 100M。数据条数一般是根据文档的大小和服务器性能而定的，但是单次批处理的数据大小应从 5MB～15MB 逐渐增加，当性能没有提升时，把这个数据量作为最大值。

2. 优化存储设备：使用固态硬盘或者磁盘阵列。
3. 合理使用合并

Lucene 以段的形式存储数据。随着数据量的变化，段的数量会越来越多，消耗的多文件句柄数及 CPU 就越多，查询效率就会下降,ES让后台定期进行段合并。

当段合并的速度落后于索引写入的速度时，ES 会把索引的线程数量减少到 1。
...
4. 减少 `Refresh` 的次数: `index.refresh_interval:30s`

`Lucene` 在新增数据时，采用延迟写入的策略，默认情况下索引的 `refresh_interval` 为 1 秒。
...
2. 仅全文搜索：对字符串字段全部进行分词索引，不进行聚合、排序、精确匹配。
...
| 参数         | 备注                                                                                                    |
|------------|-------------------------------------------------------------------------------------------------------|
| doc_values | 默认为 `true` <br />当 `fasle` 时，无法基于该字段排序、聚合、在脚本中访问字段值。<br />当 `true` 时，会增加一个正排索引，增加磁盘的占用，也会导致索引数据速度慢一些。 |
| store      | 默认为 `false` , 默认存储在`_source`字段 <br />当`true` 时，会独立于`_source`建立额外的存储，如果需要从文档中提取，会减少计算                  |                                |

4. `boolean`：可存储布尔值，也可储存"true"、"false"的字符串，不可储存0、1。

| 参数         | 备注                                                                                        |
|------------|-------------------------------------------------------------------------------------------|
| doc_values | 默认为 `true`,当 `fasle` 时，无法基于该字段排序、聚合、在脚本中访问字段值。当 `true` 时，会增加一个正排索引，增加磁盘的占用，也会导致索引数据速度慢一些。 |
| index      | 字段是否被索引，不被索引的字段不能作为查询条件                                                                   |
| store      | 默认为 `false` , 默认存储在`_source`字段 <br />当`true` 时，会独立于`_source`建立额外的存储，如果需要从文档中提取，会减少计算      |
...
| 参数             | 备注                                                                                                    |
|----------------|-------------------------------------------------------------------------------------------------------|
| boost          | 字段权重，改变查询结果的排序，默认为 1.0。                                                                               |
| doc_values     | 默认为 `true` <br />当 `fasle` 时，无法基于该字段排序、聚合、在脚本中访问字段值。<br />当 `true` 时，会增加一个正排索引，增加磁盘的占用，也会导致索引数据速度慢一些。 |
| format         | 可解析的日期格式。畸形日期会抛出异常并丢弃整个文档，`ignore_malformed=true`来改变这一行为                                              |
| include_in_all | 字段是否被包含在`_all` 字段中,默认为true。                                                                           |
| index          | 字段是否被索引，不被索引的字段不能作为查询条件                                                                               |
| precision_step | 指定数值字段每隔多少数值，生成一个词，step值越低，产生的词数量越高，范围查询越快，索引越大，占用存储空间越大，默认值为 16。                                     |
| store          | 默认为 `false` , 默认存储在`_source`字段 <br />当`true` 时，会独立于`_source`建立额外的存储，如果需要从文档中提取，会减少计算                  |

6. `object`、`nested`：对象和嵌套对象，可通过 `nested` 关键字对 `nested` 字段类型进行搜索。
7. `range`：该类型细分为`integer_range`、`float_range`、`long_range`、`double_range`、`date_range`、`ip_range`，d`ate_range`不管什么格式都被解析为毫秒进行比较大小。
8. `numbers`：该类型细分为`long`、`integer`、`short`、`byte`、`double`、`float`、`half_float`、`unsigned_long`。

| 参数               | 备注                                                                                                    |
|------------------|-------------------------------------------------------------------------------------------------------|
| coerce           | 默认值是true，尝试将字符串型数据转换为整数型数字数据。                                                                         |
| boost            | 字段权重，改变查询结果的排序，默认为 1.0                                                                                |
| doc_values       | 默认为 `true` <br />当 `fasle` 时，无法基于该字段排序、聚合、在脚本中访问字段值。<br />当 `true` 时，会增加一个正排索引，增加磁盘的占用，也会导致索引数据速度慢一些。 |
| ignore_malformed | 如果是 `true`，畸形的数字会被忽略。<br />如果是 `false`（默认），畸形数字会抛出异常并丢弃整个文档。                                          |
| include_in_all   | 字段是否被包含在`_all` 字段中,默认为true。                                                                           |
| index            | 字段是否被索引，不被索引的字段不能作为查询条件                                                                               |
| precision_step   | 指定数值字段每隔多少数值，生成一个词，step值越低，产生的词数量越高，范围查询越快，索引越大，占用存储空间越大。<br />默认值取决于数字类型。                            |
| store            | 默认为 `false` , 默认存储在`_source`字段 <br />当`true` 时，会独立于`_source`建立额外的存储，如果需要从文档中提取，会减少计算                  |
...
12. `text`：在被搜索之前通过分词器进行分词，然后形成倒排索引，不适合排序、聚合计算。
 ...
 | 参数                     | 备注                                                                                                                                  |
 |------------------------|-------------------------------------------------------------------------------------------------------------------------------------|
 | analyzer               | 分词器。<br />默认为索引分词器，或者标准分词器。                                                                                                         |
 | boost                  | 字段权重，改变查询结果的排序，默认为 1.0                                                                                                              |
 | fielddate              | 默认禁用，是否可以使用内存数据结构排序，消耗大量的堆空间                                                                                                        |
 | ignore_above           | 配合`keyword`类型使用，超过`ignore_above`长度的字符串，`analyzer`不会进行索引分析，默认为 0（禁用）。                                                                |
 | include_in_all         | 字段是否被包含在`_all` 字段中,默认为`true`。                                                                                                       |
 | index                  | 字段是否被索引，不被索引的字段不能作为查询条件                                                                                                             |
 | index_options          | 添加索引的额外信息，例如词频、偏移量等信息。                                                                                                              |
 | norms                  | 计算查询得分的时候是否应该考虑字段长度。                                                                                                                |
 | null_value             | 接受一个字符串值替换所有 `null` 值。默认为 `null`，意味着字段被作为缺失字段。 `null` 值不能被索引或者搜索。                                                                   |
 | position_increment_gap | 在字符串数组插入多少个虚拟词，默认值为100。<br /> 例如 `[ "John Abraham", "Lincoln Smith"]`中`Abraham`和`Lincoln`中还存在100个词，使用 `slop=1` 不能搜索到，需要使用`slop=101` |
 | store                  | 默认为 `false` , 默认存储在`_source`字段 <br />当`true` 时，会独立于`_source`建立额外的存储，如果需要从文档中提取，会减少计算                                                |
 | search_analyzer        | 指定搜索时的分词器。                                                                                                                          |
 | search_quote_analyzer  | 指定搜索短语时的分词器。                                                                                                                        |
...
* `match_all`：可以查询到所有文档，是没有查询条件下的默认语句。
  ...
  ```
* `match_phrase`：对查询条件分词后，将分词结果进行倒排索引匹配，查询结果必须包含所有分词且顺序必须一样，可通过 `slop` 额外溢出几个单词也可以命中，例如：
...
排序中加入`_id`,是为了在 `order_date` 一样的情况下使用 `_id` 排序，在查询结果中会返回：
...
这里的`scroll`参数，描述的是快照保存在search context的时间。上面的查询结果会返回一个 `scroll_id` ，用于后面的查询。
